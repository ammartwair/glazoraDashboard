...
function hashPassword(pw) {
  return crypto.createHash('sha256').update(pw + 'glazora_salt').digest('hex');
}
...
router.post('/send-otp', otpLimiter, async (req, res) => {
  try {
    ...
    if (!['email', 'phone'].includes(contactType)) {
      return res.status(400).json({ success: false, message: 'contactType must be email or phone' });
    }
    ...
    if (contactType === 'email') {
      await sendOTPEmail({ to: contact, otp, lang });
    }
    // For phone: integrate SMS provider here (Twilio / Unifonic)

    // In development, return OTP in response for testing
    const devExtra = process.env.NODE_ENV === 'development' ? { _dev_otp: otp } : {};
